...
public class UserDaoImpl implements UserDao {
    ...
    public final int login(String sessionId, String login, String password) {
        ...
        try (final var session = getSession()) {
            try {
                final var mapper = session.getMapper(UserMapper.class);
                final var userId = mapper.getUserIdByLoginAndPassword(login, password);
    ...
    public int hasPermissions(String sessionId) {
        ...
        try (final var session = getSession()) {
            final var userId = session.getMapper(UserMapper.class).hasPermissions(sessionId);
